...
u8 usmart_str2float(u8*str,u32 *res,float *fres,u8 flen){
	u8 ret;
	u32 pes=1;
	ret=usmart_str2num(str,res);
	//依靠flen算出被除数
	if(ret==0){
...
u8 usmart_float(u32 *psc,u8 flen){
	*psc=1;
	for(int i=0;i<flen;i++){
			*psc=(*psc)*10;
		}
...
//返回值:0,成功;其他,错误代码.
u8 usmart_get_fname(u8*str,u8*fname,u8 *pnum,u8 *rval,u8 source)
{
	u8 res;
	u8 fover=0;	  //括号深度
	u8 *strtemp;
	...
	u8 pcnt=0;	 //参数计数器
	u8 nchar;
	//判断str来源是本地的还是USART传过来的
	if(source==SOURCE_LOCAT){
		//判断函数是否有返回值
		strtemp=str;
...
u8 usmart_get_aparm(u8 *str,u8 *fparm,u8 *ptype,u8 *flen)
{
	u8 i=0;
	u8 enout=0;
	u8 type=0;//默认是数字
	...
	{		    
		...
		if(type==0||type==3)//默认是数字的
		{
			if(*str=='.'){
				type=3;		
			}else if((*str>='0' && *str<='9')||*str=='-'||*str=='+'||(*str>='a' && *str<='f')||(*str>='A' && *str<='F')||*str=='X'||*str=='x')//数字串检测
			{
				if(enout)break;					//找到了下一个参数,直接退出.
				if(*str>='a')*fparm=*str-0X20;	//小写转换为大写
				else if(fl!=3){					//只取小数点后3位
...
{	
	...
	while(1)
	{
		i=usmart_get_aparm(str,tstr,&type,&flen);	//得到第一个参数  
		str+=i;								//偏移
		switch(type)
		{
			case 0:	//数字
			...
			case 3:	//浮点数字
				...
				{					    
					i=usmart_str2num(tstr,&res);	//记录该参数	
					usmart_float(&(usmart_dev.fparm[n]),flen);//记录浮点被除数					
					if(i)return USMART_PARMERR;		//参数错误.
					*(u32*)(usmart_dev.parm+usmart_get_parmpos(n))=res;//记录转换成功的结果.
					usmart_dev.parmtype&=~(1<<n);	//标记数字
					usmart_dev.numtype|=1<<n;    //标记浮点类型
					usmart_dev.plentbl[n]=4;		//该参数的长度为4  
					n++;							//参数增加  
					if(n>MAX_PARM)return USMART_PARMOVER;//参数太多
				}
