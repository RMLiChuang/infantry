...
u8 usmart_str2float(u8*str,u32 *res,float *fres,u8 flen); //字符串转为浮点数字
u8 usmart_float(u32 *psc,u8 flen);                  //获取浮点被除数
u8 usmart_get_cmdname(u8*str,u8*cmdname,u8 *nlen,u8 maxlen);//从str中得到指令名,并返回指令长度
u8 usmart_get_fname(u8*str,u8*fname,u8 *pnum,u8 *rval,u8 source);		//从str中得到函数名
u8 usmart_get_aparm(u8 *str,u8 *fparm,u8 *ptype,u8 *flen); 	//从str中得到一个函数参数
u8 usmart_get_fparam(u8*str,u8 *parn);  			//得到str中所有的函数参数.
u8 usmart_get_fname_usart(u8*str,u8*fname,u8 *pnum);		//从usart的str中得到函数名
#endif
