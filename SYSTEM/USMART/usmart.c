...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 3://查询ID
			...
			{
				usmart_get_fname((u8*)usmart_dev.funs[i].name,sfname,&pnum,&rval,SOURCE_LOCAT);//得到本地函数名 
		...
		case 4://hex指令
			printf("\r\n");
			usmart_get_aparm(str,sfname,&i,0);
			if(i==0)//参数正常
			{
...
{ 
    ...
    HAL_NVIC_SetPriority(TIM4_IRQn,2,2);    //设置中断优先级，抢占优先级3，子优先级3
...
u8 usmart_cmd_rec(u8*str) 
{
	...
	u8 sfname[MAX_FNAME_LEN];//存放本地函数名
	sta=usmart_get_fname(str,rfname,&rpnum,&rval,SOURCE_USART);//得到接收到的数据的函数名及参数个数	  
	if(sta)return sta;//错误
	for(i=0;i<usmart_dev.fnum;i++)
	{
		sta=usmart_get_fname((u8*)usmart_dev.funs[i].name,sfname,&spnum,&rval,SOURCE_LOCAT);//得到本地函数名及参数个数
...
void usmart_exe(void)
{
	...
	usmart_get_fname((u8*)usmart_dev.funs[id].name,sfname,&pnum,&rval,SOURCE_LOCAT);//得到本地函数名,及参数个数 
	printf("\r\n%s(",sfname);//输出正要执行的函数名
	for(i=0;i<pnum;i++)//输出参数
	{
		...
		}else if(usmart_dev.numtype&(1<<i))						  //参数是浮点数字
		{
			temp[i]=*(u32*)(usmart_dev.parm+usmart_get_parmpos(i));
			fparm=(float)temp[i]/(float)usmart_dev.fparm[i];
			if(usmart_dev.sptype==SP_TYPE_DEC)printf("%.4f",fparm);//10进制参数显示
