...
u8 MPU_Get_Gyroscope(mpu_data_t *mpu9250)
{
	...
	if(res==0)
	{
		mpu9250->gx=((u16)buf[0]<<8)|buf[1];  
		mpu9250->gy=((u16)buf[2]<<8)|buf[3];  
		mpu9250->gz=((u16)buf[4]<<8)|buf[5];
	} 	
    return res;
}
...
u8 MPU_Get_Accelerometer(mpu_data_t *mpu9250)
{
	...
	if(res==0)
	{
		mpu9250->ax=((u16)buf[0]<<8)|buf[1];  
		mpu9250->ay=((u16)buf[2]<<8)|buf[3];  
		mpu9250->az=((u16)buf[4]<<8)|buf[5];
	} 	
    return res;;
}
...
u8 MPU_Get_Magnetometer(mpu_data_t *mpu9250)
{
	...
	if(res==0)
	{
		mpu9250->mx=((u16)buf[1]<<8)|buf[0];  
		mpu9250->my=((u16)buf[3]<<8)|buf[2];  
		mpu9250->mz=((u16)buf[5]<<8)|buf[4];
	} 	
